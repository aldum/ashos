...
def aur_install(snap, skip_prep=False, skip_post=False):
    ...
    if options["aur"] == "True":
        ...
        if aur and not aur_check(snap):
            if not skip_prep:
                core.prepare(snap)  ### REVIEW NEEDED? Being called twice!
...
def aur_install_helper(snap):
    ...
    os.system(
        f"chroot /.snapshots/rootfs/snapshot-chr{snap} chown -R aur /home/aur{DEBUG}"
    )
    # TODO: more checking here
    excode = os.system(
        f"chroot /.snapshots/rootfs/snapshot-chr{snap} su aur -c 'rm -rf /home/aur/paru-bin && cd /home/aur && git clone https://aur.archlinux.org/paru-bin.git'{DEBUG}"
    )
    if excode:
...
def aur_install_live_helper(snap):
    ...
    excode = os.system(
        f"chroot /.snapshots/rootfs/snapshot-{snap} pacman -S --noconfirm --needed sudo git base-devel{DEBUG}"
    )
    if excode:
        return excode
    ...
    os.system(
        f"chroot /.snapshots/rootfs/snapshot-{snap} chown -R aur /home/aur{DEBUG}"
    )
    # TODO: no error checking here
    excode = os.system(
        f"chroot /.snapshots/rootfs/snapshot-{snap} su aur -c 'rm -rf /home/aur/paru-bin && cd /home/aur && git clone https://aur.archlinux.org/paru-bin.git'{DEBUG}"
    )
    if excode:
        print("F: failed to download paru-bin")
        return excode
    excode = os.system(
        f"chroot /.snapshots/rootfs/snapshot-{snap} su aur -c 'cd /home/aur/paru-bin && makepkg --noconfirm -si{DEBUG}'"
    )
    if excode:
        print("F: failed installing paru-bin")
        return excode
...
def auto_upgrade(snap):
    ...
    if not aur:
        excode = os.system(
            f"chroot /.snapshots/rootfs/snapshot-chr{snap} pacman --noconfirm -Syyu"
        )
    else:
        excode = os.system(
            f"chroot /.snapshots/rootfs/snapshot-chr{snap} su aur -c 'paru --noconfirm -Syy'"
        )
    if excode == 0:
        post_transactions(snap)
...
def cache_copy(snap, FROM):
    os.system(
        f"cp -n -r --reflink=auto /.snapshots/rootfs/snapshot-chr{snap}/var/cache/pacman/pkg/. /var/cache/pacman/pkg/{DEBUG}"
    )
    # if aur_enabled:
...
def fix_package_db(snap=0):
    ...
    if not os.path.exists(f"/.snapshots/rootfs/snapshot-{snap}"):
        print(
            f"F: Cannot fix package manager database as snapshot {snap} doesn't exist."
        )
        return
    elif os.path.exists(f"/.snapshots/rootfs/snapshot-chr{snap}"):
        print(
            f"F: Snapshot {snap} appears to be in use. If you're certain it's not in use, clear lock with 'ash unlock -s {snap}'."
        )
        return
    elif snap == 0:
    ...
    try:
        ...
        os.system(
            "rm -rf /etc/pacman.d/gnupg $HOME/.gnupg"
        )  ### $HOME vs /root NEEDS fixing # If folder not present and sp.run is used, throws error and stops
...
def install_package(pkg, snap):
    core.prepare(snap)
    excode = os.system(
        f"chroot /.snapshots/rootfs/snapshot-chr{snap} pacman -S {pkg} --needed --overwrite '/var/*'"
    )
    if excode:
        aur = aur_install(
            snap, True, True
        )  ### TODO: do a paru -Si {pkg} check to avoid setup_aur if package already installed!
        if aur:
            return os.system(
                f"chroot /.snapshots/rootfs/snapshot-chr{snap} su aur -c \"paru -S {pkg} --needed --overwrite '/var/*'\""
            )
        else:
            ...
            if yes_no("Enable AUR?"):
                ...
                snapshot_config_edit(
                    snap, True, False
                )  ### TODO move to core.py ? Run prepare but skip post transaction (Optimize code) Update: post_tran need to run too
...
def install_package_old(pkg, snap):
    try:
        ...
        sp.check_output(
            f"pacman -Si {pkg}", shell=True, stderr=sp.PIPE
        )  # --sysroot ### do not print if pkg not found
    except sp.CalledProcessError:
        aur = aur_install(
            snap
        )  ### TODO: do a paru -Si {pkg} check to avoid setup_aur if package already installed!
    ...
    try:
        ...
        if aur_in_tmp and not aur_check(tmp):
            ...
            if excode:
                ...
                os.system(f"umount /.snapshots/rootfs/snapshot-{tmp}{DEBUG}")
                print(
                    "F: Live installation failed!"
                )  # Before: Live install failed and changes discarded
                return excode
        ...
        excode = os.system(
            f"chroot /.snapshots/rootfs/snapshot-{tmp} pacman -Sy --overwrite '*' --noconfirm {pkg}{DEBUG}"
        )  ### REVIEW Maybe just do this in try section and remove else section!
    return excode
...
def pkg_list(snap, CHR=""):
    return (
        sp.check_output(
            f"chroot /.snapshots/rootfs/snapshot-{CHR}{snap} pacman -Qq",
            encoding="utf-8",
            shell=True,
        )
        .strip()
...
def presets_helper(prof_cp, snap):  ### TODO before: prof_section
    if prof_cp.has_option("presets", "enable_aur"):
        ...
        aur_install(
            snap, False, False
        )  ### Skip prepare, but run post transaction (Optimize code) ### update: because of last step, had to run prepare again too!
...
def snapshot_diff(snap1, snap2):
    if not os.path.exists(f"/.snapshots/rootfs/snapshot-{snap1}"):
        ...
        os.system(
            f"bash -c \"diff <(ls /.snapshots/rootfs/snapshot-{snap1}/usr/share/ash/db/local) <(ls /.snapshots/rootfs/snapshot-{snap2}/usr/share/ash/db/local) | grep '^>\\|^<' | sort\""
        )  ### REVIEW
...
def uninstall_package_helper(pkg, snap):
    return os.system(
        f"chroot /.snapshots/rootfs/snapshot-chr{snap} pacman --noconfirm -Rns {pkg}"
    )
...
def upgrade_helper(snap):
    ...
    core.prepare(
        snap
    )  ### REVIEW tried it outside of this function in ashpk_core before aur_install and it works fine!
    if not aur:
        excode = os.system(f"chroot /.snapshots/rootfs/snapshot-chr{snap} pacman -Syyu")
    else:
        excode = os.system(
            f"chroot /.snapshots/rootfs/snapshot-chr{snap} su aur -c 'paru -Syyu'"
        )
    return excode
