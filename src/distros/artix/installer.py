...
import sys
import src.installer_core as core
...
is_efi = core.is_efi
installer_dir = core.installer_dir
...
def main():
    ...
    while True:
        try:
            ...
            print(e)
            if not core.yes_no("F: Failed to strap package(s). Retry?"):
                core.unmounts("failed")  # user declined
                sys.exit("F: Install failed!")
        else:  # success
            break

    #   Go inside chroot
    cur_dir_code = core.chroot_in("/mnt")

    #   3. Package manager database and config files
    os.system(
        "cp -r /var/lib/pacman/. /usr/share/ash/db/"
    )  # removed /mnt/XYZ from both paths and below
    os.system(
    ...
    )  # removed /mnt/XYZ from both paths (and from all lines above)
    ...
    #   BTRFS snapshots
    core.deploy_base_snapshot()

    #   Copy boot and etc: deployed snapshot <---> common
    core.deploy_to_common()

    #   Unmount everything and finish
    core.chroot_out(cur_dir_code)
    if is_ash_bundle:
        core.bundler()
    core.unmounts()
...
def initram_update():  # REVIEW removed "{SUDO}" from all lines below
    if is_luks:
        os.system(
            "dd bs=512 count=4 if=/dev/random of=/etc/crypto_keyfile.bin iflag=fullblock"
        )  # removed /mnt/XYZ from output (and from lines below)
        os.system(
            "chmod 000 /etc/crypto_keyfile.bin"
        )  # Changed from 600 as even root doesn't need access
        os.system(f"cryptsetup luksAddKey {args[1]} /etc/crypto_keyfile.bin")
        os.system(
            "sed -i -e '/^HOOKS/ s/filesystems/encrypt filesystems/' \
                        -e 's|^FILES=(|FILES=(/etc/crypto_keyfile.bin|' /etc/mkinitcpio.conf"
        )
    if is_format_btrfs:  # REVIEW temporary
        os.system(
            "sed -i 's|^MODULES=(|MODULES=(btrfs|' /etc/mkinitcpio.conf"
        )  # TODO if array not empty, needs to be "btrfs "
    if (
        is_luks or is_format_btrfs
    ):  # REVIEW mkinitcpio needed to run without these conditions too?
